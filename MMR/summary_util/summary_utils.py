...
SUM_UTILS_DIR = os.path.dirname(os.path.abspath(__file__))
STEMMED_IDF_FILE = SUM_UTILS_DIR+'/bgIdfValues.stemmed.txt'
UNSTEMMED_IDF_FILE = SUM_UTILS_DIR+'/bgIdfValues.unstemmed.txt'
...
def is_redundant(sal_dict, doc_id, sent_id,cluster, lamb, mini, sent, sents, vect_fun=tfidf_vectorize):
    """Given a tokenized sentence and a list of tokenized sentences,
    return whether the sentences overlaps too highly in content with any
    of the others."""

    #print float(sal_dict[cluster][doc_id][sent_id]) * 100
    current_max_sim=0
    for other_sent in sents:
        feat_space = feature_space(sent, other_sent)
        x, y = vect_fun(feat_space, sent), vect_fun(feat_space, other_sent)
        #print cosine_sim(x, y)
        if cosine_sim(x, y) > current_max_sim:
            current_max_sim = cosine_sim(x, y)
    MMR = (lamb * (float(sal_dict[cluster][doc_id][sent_id]) * 10) ) -  ((1-lamb) * (current_max_sim)) 
    #print MMR
    if MMR  < mini:
        return True     
    return False
