...
var pageRank = function() {
	...
	// outer loop for one iteration of page rank calculation
	for (var i = 0; i < n; i++) {
		var thisNode = nodes[i].id;

		newPR[thisNode] = (1 - d) / n;
		newPR[thisNode] += d * sinkPR / n;
	...
	for (var i = 0; i < n; i++) {
		...
		var thisPR = pageranks[thisNode];
		var thisOutDeg = outlinks[thisNode].length;

		// you don't need the outdegree above for divrank
		// var thisOutDeg = 0;
		// for (var j=0; j < outlinks[thisNode].length; j++) {
			// var thisOut = outlinks[thisNode][j];
			// thisOutDeg += pageranks[thisOut];
			// }

		for (var j=0; j < outlinks[thisNode].length; j++) {

			var thisOut = outlinks[thisNode][j];
			newPR[thisOut] += d * thisPR / thisOutDeg;
			// newPR[thisOut] += thisPR * pageranks[thisOut] / thisOutDeg;
