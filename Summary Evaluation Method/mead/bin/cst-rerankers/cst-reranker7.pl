...
#    }
    ...
    ### Ajust scores by source (increasing authority)
    foreach my $sentref (@all_sents) {
            $$sentref{'Score'} += 
($SourcePriorityHash{$SourceHash{$$sentref{'DID'}}}-$MinSource)/($MaxSource-$MinSource);  
		#'SNO' for sent no.
    }
...
sub ParseSentRelFile
{
  ...
  $RelationTree->parsefile($RelationFile) or die "error parsing sent rel file";
